mainApp
		.controller(
				...
				function($scope, $http, $window, $rootScope, Excel, $timeout,$compile) {
					...
					$scope.getSurveyReports = function getSurveyReports() {
						...
						$http.get(baseUrl + 'getAdminsSurveyReports/').success(
								function(data) {
									...
									$scope.superAdminSurveyData = data;
					...
					$scope.numberOfDeletedSurveyPages = function numberOfDeletedSurveyPages() {
						if ($scope.deletedServeyReport != null) {
							return Math.ceil($scope.deletedServeyReport.length
									/ $scope.pageSize);
						}
					}

					$scope.numberOfSurveyReportPages = function numberOfSurveyReportPages() {
						if ($scope.superAdminSurveyReport != null) {
							return Math
									.ceil($scope.superAdminSurveyReport.length
											/ $scope.pageSize);
						}
					}
					...
					$scope.resetAll = function resetAll() {
						$scope.searchData = {};
						$scope.superAdminSurveyReport = $scope.tempSurveyReport;
...
/*****************************************************************Function for addSlumData****************************************************************/
					...
					$scope.saveSlumSurvey = function saveSlumSurvey(addSlumSurvey,clickButton) {
						...
						var surveyData = {
							...
							"chckSlumRadio":$scope.addSurvey.chckSlumRadio,
						...
						$http
								...
								.success(
										function(surveyReport) {
											...
											if (surveyReport != "") {
												$scope.superAdminSurveyReport = surveyReport;
											} else {
												swal({
													position : 'top',
