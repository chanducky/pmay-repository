mainApp
		.controller(
				...
				function($scope, $http, $window, $rootScope, Excel, $timeout,$compile) {
					...
					$scope.getSurveyReports = function getSurveyReports() {
						...
						$http.get(baseUrl + 'getAdminsSurveyReports/').success(
								function(data) {
									...
									$scope.adminSurveyReport = data;
									$scope.adminSurveyData = data;
					...
					$scope.numberOfAdminSurveyReportPages = function numberOfAdminSurveyReportPages() {
						if ($scope.adminSurveyReport != null) {
							return Math.ceil($scope.adminSurveyReport.length
									/ $scope.pageSize);
						}
					}

					$scope.numberOfAdminSurveyDataPages = function numberOfAdminSurveyDataPages() {
						if ($scope.adminSurveyData != null) {
							return Math.ceil($scope.adminSurveyData.length
									/ $scope.pageSize);
						}
					}

					$scope.numberOfAdminPendingRequestPages = function numberOfAdminPendingRequestPages() {
						if ($scope.pendingUsers != null) {
							return Math.ceil($scope.pendingUsers.length
									/ $scope.pageSize);
						}
					}
					...
					$scope.resetAll = function resetAll() {
						$scope.searchData = {};
						$scope.adminSurveyReport = $scope.tempSurveyData;
						...
						$http
								...
								.success(
										function(surveyReport) {
											...
											if (surveyReport != "") {
												$scope.adminSurveyReport = surveyReport;
											} else {
												swal({
													position : 'top',
...
/************************functions for some validation**************************/
					...
					$scope.numberOfUserDetailsReportPages = function numberOfUserDetailsReportPages() {
						if ($scope.userDetailsReport != null) {
							return Math.ceil($scope.userDetailsReport.length / $scope.pageSize);
						}
					}
